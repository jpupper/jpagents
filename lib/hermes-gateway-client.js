...
export function resolveGatewayConfig() {
    ...
    // 2. API key: SIEMPRE intentar leer de .env
    let apiKey = '';
    try {
                ...
                const req = requester.request(url, {
                    ...
                    res.on('end', () => {
                        ...
                        try {
                            ...
                            // 🚨 Detectar respuesta vacía también en modo no-streaming
                            const isEmptyCompletion = usage &&
                                usage.totalTokens === 0 &&
                                !content.trim();
                            if (isEmptyCompletion) {
                                const suggestion = 'Recargá tu cuenta en https://platform.deepseek.com o verificá tu API key.';
                                const errorMsg = `⚠️ El modelo upstream (${model}) no generó respuesta (0 tokens, contenido vacío). Esto suele ocurrir cuando la API key no tiene saldo suficiente o hay un error de configuración. ${suggestion}`;
        ...
        return new Promise((resolve, reject) => {
            ...
            const req = requester.request(url, {
                ...
                res.on('end', () => {
                    ...
                    // 🚨 Detectar respuesta vacía: 0 tokens y sin contenido.
                    // El Gateway a veces se traga errores de la API upstream
                    // (como "Insufficient Balance", API key inválida, etc.) y
                    // devuelve HTTP 200 con una respuesta SSE vacía.
