...
export function formatMessage(content) {
    ...
    text = text.replace(/(`[^`]+`)/g, (match) => {
        return stash(match.replace(/\\/g, '\\\\'));
    });

    // 3) Convertir links [text](url)
    ...
    const safeParts = codeSplit.map((seg, idx) => {
        ...
        return seg.replace(/[(){}]/g, (ch) => {
            // ( que abre link [text](url)
            if (ch === '(') {
                const before = seg.slice(0, seg.indexOf(ch));
            ...
            if (ch === ')') {
                const before = seg.slice(0, seg.indexOf(ch));
                if (before.includes('](http') || before.includes('](')) return ch;
                return '\\)';
            }
            return '\\' + ch;
        });
