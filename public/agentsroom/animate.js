...
import { scene, camera, renderer, labelRenderer, controls, ambientParticles, ambientSpeeds, particleCount } from './scene.js';
import { S } from './state.js';
import { spawnCharParticle } from './effects.js';
import { cleanupExitedAgents, spawnArcaneBubble } from './scene-manager.js';

const clock = new THREE.Clock();

// ─── COMPUTE CENTER SUMMON INTENSITY ───
function computeCenterSummonIntensity() {
  let totalActive = 0;
  for (const [, entry] of S.agentMap) {
    if (entry.exiting) continue;
    const st = entry.agentData?.status;
    if (st === 'running' || st === 'thinking') totalActive++;
  }
  // Intensity grows with active count but saturates
  const raw = totalActive / Math.max(1, S.agentMap.size);
  const intensity = Math.min(1, raw * 1.5);
  S.centerSummonIntensity += (intensity - S.centerSummonIntensity) * 0.05;
}

// ─── ANIMATE CENTER SUMMON MARK ───
function animateCenterSummon(t) {
  const cs = S.centerSummonGroup;
  if (!cs) return;
  const intensity = S.centerSummonIntensity;
  const opacity = intensity * 0.6;

  // Base glow disc
  cs.baseMat.opacity = opacity * 0.4;

  // Concentric rings - rotate at different speeds
  cs.rings.forEach((ring, i) => {
    ring.material.opacity = opacity * (0.3 + i * 0.12);
    ring.rotation.z += 0.005 * ring.userData.speed * (0.5 + intensity * 0.5);
    const pulse = 1 + Math.sin(t * 0.5 + i * 1.2) * intensity * 0.06;
    ring.scale.set(pulse, pulse, 1);
  });

  // Outer glow ring
  cs.glowRing.material.opacity = opacity * 0.15 * (0.5 + Math.sin(t * 0.3) * 0.5);
  cs.glowRing.rotation.z += 0.002 * intensity;

  // Inner rune circle
  cs.runeCircle.material.opacity = opacity * 0.5 * (0.6 + Math.sin(t * 0.7) * 0.4);
  cs.runeCircle.rotation.z = t * 0.08 * intensity;

  // Rune sprites
  cs.runeSprites.forEach((rune) => {
    const ud = rune.userData;
    const angle = ud.angle + t * ud.speed * intensity;
    rune.position.x = Math.cos(angle) * ud.radius;
    rune.position.z = Math.sin(angle) * ud.radius;
    rune.position.y = ud.yBase + Math.sin(t * 0.5 + ud.phase) * 2 * intensity;
    rune.material.opacity = opacity * 0.7 * (0.5 + Math.sin(t * 0.4 + ud.phase) * 0.5);
    const s = (6 + Math.sin(t * 0.6 + ud.phase) * 2) * intensity;
    rune.scale.set(s, s, 1);
  });

  // Energy pillar
  cs.pillar.material.opacity = opacity * 0.08 * (0.5 + Math.sin(t * 0.4) * 0.5);

  // Inner sphere
  cs.sphere.material.opacity = opacity * 0.5 * (0.5 + Math.sin(t * 0.8) * 0.5);
  cs.sphere.scale.setScalar(1 + Math.sin(t * 0.6) * 0.15 * intensity);
}

// ─── COMPUTE ACTIVE AGENTS FOR CENTER MARK ───
let centerTotalActive = 0;
...
export function animate() {
  ...
  // ─── COMPUTE CENTER SUMMON INTENSITY ───
  computeCenterSummonIntensity();
  animateCenterSummon(t);

  // ─── PEDESTAL ORBIT CAMERA ───
  if (S.isPedestalMode && S.pedestalState) {
    const ps = S.pedestalState;

    if (ps.animating) {
      ps.animT += dt * 0.6;
      const p = Math.min(ps.animT, 1);
      const ease = 1 - Math.pow(1 - p, 3);

      camera.position.lerpVectors(ps.initialCamPos, ps.targetPos, ease);
      controls.target.lerpVectors(ps.initialTarget, ps.pedestalCenter, ease);
    ...
    if (!ps.animating) {
      // Auto-rotate YAW around pedestal
      ps.currentAngle += dt * 0.25;
      const radius = 120;
      const height = 40;
      const cx = ps.pedestalCenter.x;
      const cz = ps.pedestalCenter.z;
      const cy = ps.pedestalCenter.y + 15;

      camera.position.set(
        cx + Math.cos(ps.currentAngle) * radius,
        cy + height,
        cz + Math.sin(ps.currentAngle) * radius
      );
      controls.target.copy(ps.pedestalCenter);
    }
  }
  ...
  if (S.focusState && S.focusState.animating) {
    ...
    // Face the agent from the FRONT — camera between agent and orbit center
    // The mage faces toward orbitCenter, so "front" = between mage and center
    const orbitCenter = S.focusState.mage.userData.orbitCenter;
    if (orbitCenter) {
      const dir = new THREE.Vector3()
        .subVectors(camTarget, orbitCenter)
        .normalize();
      // Camera position: IN FRONT of the agent (between agent and orbit center)
      // Negative dir = toward orbit center = face-to-face with the mage
      const dist = 55;
      const targetPos = new THREE.Vector3(
        camTarget.x - dir.x * dist,
        camTarget.y + 20,
        camTarget.z - dir.z * dist
      );
  ...
  S.mageGroup.children.forEach((mage) => {
    ...
    // Don't orbit if in focus mode for this agent
    if (S.isFocusMode && S.focusState && S.focusState.mage === mage) return;
