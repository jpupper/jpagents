...
let currentCustomizingAgentId = null;
...
function focusAgent(agentIndex) {
  ...
  buildCustomizationUI(agentData, targetMage);
}
...
// ─── CUSTOMIZATION: LOAD SAVED COLORS ───
function getCustomColors(agentId) {
  try {
    const saved = localStorage.getItem('agentsroom-custom-colors');
    if (saved) {
...
function saveCustomColors(agentId, colors) {
  try {
    const saved = localStorage.getItem('agentsroom-custom-colors');
    let all = {};
    if (saved) try { all = JSON.parse(saved); } catch(e) {}
    all[agentId] = colors;
    S.customColors[agentId] = colors;
    localStorage.setItem('agentsroom-custom-colors', JSON.stringify(all));
  } catch (e) {}
}

// ─── APPLY CUSTOM COLORS TO MAGE ───
function applyCustomColors(agentId, mage) {
  if (!mage || !agentId) return;
  const colors = getCustomColors(agentId);
  if (!colors) return;

  // The mage group has specific children at known positions
  // body: child with cy10x14x30 geometry
  mage.traverse(child => {
    if (!child.isMesh || !child.material) return;

    // Body (largest cylinder)
    if (child.geometry.type === 'CylinderGeometry' &&
        child.geometry.parameters.radiusTop === 10 &&
        child.geometry.parameters.radiusBottom === 14 &&
        child.geometry.parameters.height === 30) {
      if (colors.body) child.material.color.set(colors.body);
    }

    // Hat cone
    if (child.geometry.type === 'ConeGeometry' &&
        child.geometry.parameters.radius === 10 &&
        child.geometry.parameters.height === 22) {
      if (colors.hat) child.material.color.set(colors.hat);
    }

    // Hat brim
    if (child.geometry.type === 'CylinderGeometry' &&
        child.geometry.parameters.radiusTop === 14 &&
        child.geometry.parameters.radiusBottom === 14 &&
        child.geometry.parameters.height === 1.5) {
      if (colors.hat) child.material.color.set(colors.hat);
    }

    // Head (sphere radius 6)
    if (child.geometry.type === 'SphereGeometry' &&
        child.geometry.parameters.radius === 6) {
      if (colors.head) child.material.color.set(colors.head);
    }

    // Eyes (small spheres)
    if (child.geometry.type === 'SphereGeometry' &&
        child.geometry.parameters.radius === 1.8) {
      if (colors.eyes) child.material.color.set(colors.eyes);
      if (colors.eyes) child.material.emissive.set(colors.eyes);
    }

    // Beard (cone radius 5 height 8)
    if (child.geometry.type === 'ConeGeometry' &&
        child.geometry.parameters.radius === 5 &&
        child.geometry.parameters.height === 8) {
      if (colors.beard) child.material.color.set(colors.beard);
    }

    // Trim (torus)
    if (child.geometry.type === 'TorusGeometry' &&
        child.geometry.parameters.radius === 12) {
      if (colors.trim) child.material.color.set(colors.trim);
    }

    // Arms (cylinder 2, 2.5, 14)
    if (child.geometry.type === 'CylinderGeometry' &&
        child.geometry.parameters.radiusTop === 2 &&
        child.geometry.parameters.radiusBottom === 2.5 &&
        child.geometry.parameters.height === 14) {
      if (colors.arms) child.material.color.set(colors.arms);
    }

    // Aura (sphere radius 22)
    if (child.geometry.type === 'SphereGeometry' &&
        child.geometry.parameters.radius === 22 &&
        child.material.transparent) {
      if (colors.aura) child.material.color.set(colors.aura);
    }
  });
}

// ─── BUILD CUSTOMIZATION UI ───
function buildCustomizationUI(agentData, mage) {
  const agentId = agentData.id || agentData.pid;
  if (!agentId) return;
  currentCustomizingAgentId = agentId;

  const colors = getCustomColors(agentId) || {};
  const container = document.getElementById('fi-customize');
  if (!container) return;

  const parts = [
    { id: 'body', label: 'Cuerpo', default: '#2a2a4a' },
    { id: 'hat', label: 'Sombrero', default: '#1a1a3a' },
    { id: 'head', label: 'Cabeza', default: '#f0d0b0' },
    { id: 'eyes', label: 'Ojos', default: '#9966ff' },
    { id: 'beard', label: 'Barba', default: '#5a4a3a' },
    { id: 'trim', label: 'Adornos', default: '#4a3a6a' },
    { id: 'arms', label: 'Brazos', default: '#2a2a4a' },
    { id: 'aura', label: 'Aura', default: '#8844ff' },
  ];

  let html = '<div class="customize-title">✧ Personalizar Agente</div>';
  html += '<div class="customize-grid">';

  for (const part of parts) {
    const val = colors[part.id] || part.default;
    html += `
      <div class="customize-item">
        <label class="customize-label">${part.label}</label>
        <div class="customize-input-wrap">
          <input type="color" class="customize-color" data-part="${part.id}" value="${val}">
          <input type="text" class="customize-hex" data-part="${part.id}" value="${val}" maxlength="7">
        </div>
      </div>
    `;
  }
  ...
  html += '</div>';
  html += '<button class="customize-reset-btn" id="fi-custom-reset">⟲ Restaurar colores</button>';
  container.innerHTML = html;

  // Wire up color inputs
  container.querySelectorAll('.customize-color').forEach(input => {
    input.addEventListener('input', (e) => {
      const part = e.target.dataset.part;
      const hex = e.target.value;
      // Update hex input
      const hexInput = container.querySelector(`.customize-hex[data-part="${part}"]`);
      if (hexInput) hexInput.value = hex;
      // Apply immediately
      const colors = getCustomColors(agentId) || {};
      colors[part] = hex;
      saveCustomColors(agentId, colors);
      applyCustomColors(agentId, mage);
      // Rebuild the mage's emissive colors based on new body color
      updateMageEmissive(mage, colors);
    });
  });

  container.querySelectorAll('.customize-hex').forEach(input => {
    input.addEventListener('change', (e) => {
      let val = e.target.value.trim();
      if (!val.startsWith('#')) val = '#' + val;
      if (/^#[0-9a-fA-F]{6}$/.test(val)) {
        const part = e.target.dataset.part;
        const colorInput = container.querySelector(`.customize-color[data-part="${part}"]`);
  ...
  // Reset button
  const resetBtn = document.getElementById('fi-custom-reset');
  if (resetBtn) {
    resetBtn.addEventListener('click', () => {
      const existing = localStorage.getItem('agentsroom-custom-colors');
      if (existing) {
        try {
          const all = JSON.parse(existing);
          delete all[agentId];
          localStorage.setItem('agentsroom-custom-colors', JSON.stringify(all));
          delete S.customColors[agentId];
        } catch(e) {}
      }
      // Rebuild mage with original colors
      if (mage) {
        rebuildMageColors(mage);
      }
      // Rebuild UI
      buildCustomizationUI(agentData, mage);
    });
  }
}

// ─── REBUILD MAGE ORIGINAL COLORS ───
function rebuildMageColors(mage) {
  if (!mage) return;
  // Dispatch custom event for room.js to handle refresh
  window.dispatchEvent(new CustomEvent('agentsroom:reset-colors', {
    detail: { agentId: mage.userData?.agentId }
  }));
}

// ─── UPDATE MAGE EMISSIVE ───
function updateMageEmissive(mage, colors) {
  if (!mage || !colors) return;
  mage.traverse(child => {
    ...
    if (child.geometry.type === 'SphereGeometry' &&
        child.geometry.parameters.radius === 1.8) {
      if (colors.eyes) {
        child.material.emissive.set(colors.eyes);
      }
    }
    // Tip emissive
    if (child.geometry.type === 'SphereGeometry' &&
        child.geometry.parameters.radius === 2.5) {
      if (colors.hat) {
        child.material.emissive = child.material.emissive || new THREE.Color();
        child.material.emissive.set(colors.hat);
      }
    }
  });
}
...
// ─── HIDE PEDESTAL VIEW ───
function hidePedestalView() {
  // Re-export from room.js logic
}

// ─── EXPORT (add to existing) ───
export { focusAgent, unfocusAgent, updateFocusPanel, showProjectInfo, hideProjectInfo, updateProjectInfoPanel, updatePanel, buildCustomizationUI, applyCustomColors, hidePedestalView };
